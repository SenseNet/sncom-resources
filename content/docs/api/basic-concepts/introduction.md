...
| [$expand](/select-expand#expand) | allows you to identify related entries with a single URI such that a graph of entries could be retrieved with a single HTTP request (e.g. creator user or any other related content) |
...
| [$inlinecount](/collection#inlinecount) | controls the `__count` property that can be found in every collection response |
| [query](/search-filter/) | filter the result collection using sensenet Content Query |
